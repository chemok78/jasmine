...
app.use('/', (request, response) => {
  ...
  //we have the function analyse our request.body, create response and send that back
  bot(request.body, response, (error, success) => {
    //console.log(request.body);
    if (error) {
      console.log('Error in your bot:', error);
      //if response headers not send, we handle it ourselves with a 400
      if (!response.headersSent) { response.sendStatus(400) }
    } else if (success) {
      console.log(success);
      //if response headers not send, we handle it ourselves with a 200
